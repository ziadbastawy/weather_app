<template>
  <section>
    <div class="tabs">
      <button
        class="tab_btn"
        :class="{ active: viewedData == 'houlry' }"
        @click="changeViewedData('houlry')"
      >
        Hourly
      </button>
      <button
        class="tab_btn"
        :class="{ active: viewedData == 'daily' }"
        @click="changeViewedData('daily')"
      >
        Daily
      </button>
    </div>
    ...
    <div class="tabs_cards" v-else>
      <div
        class="card daily_card"
        v-for="(item, index) in dailyWeather"
        :key="index"
      >
        <p class="card_time" v-if="index !== 0">
          {{ changeDateFromTime(item.time) }}
        </p>
